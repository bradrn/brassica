...
parseTokeniseAndApplyRules_hs
    ...
    -> StablePtr (IORef (Maybe [Component [WordPart]]))  -- ^ previous results
        ...
        Right rules ->
            if report == 1 then do
                ...
                newCString $ intercalate "<br>" $ concat (getWords result) <&> \RuleApplied{..} ->
                    "<b>" ++ unWordPart input ++ "</b> &rarr;\
                    \<b>" ++ unWordPart output ++ "</b> (" ++ plaintext rule ++ ")"
            else case hlMode of
                1 -> do
                    ...
                    newCString $ escape $ detokeniseWords' id $
                        zipWithComponents result (fromMaybe [] prev) [] $ \thisWord prevWord ->
                            let thisWordStr = concat $ mapMaybe getGrapheme thisWord in
                ...
                2 -> do
                    ...
                    newCString $ escape $ flip detokeniseWords' result $ \case
                        (w, False) -> concatMap (fromRight "") w
                        (w, True) -> "<b>" ++ concatMap (fromRight "") w ++ "</b>"
                _ -> do
                    let result = tokeniseAnd applyRules cats rules wsText
                    writeIORef prevRef $ Just result
...
initResults :: IO (StablePtr (IORef (Maybe [Component [WordPart]])))
...
foreign export ccall parseTokeniseAndApplyRules_hs
    ...
    -> CBool
    -> CInt
    -> StablePtr (IORef (Maybe [Component [WordPart]]))
    -> IO CString

foreign export ccall initResults :: IO (StablePtr (IORef (Maybe [Component [WordPart]])))
